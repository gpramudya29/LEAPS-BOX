...
// ================== HELPER FUNCTIONS ==================
// Baca Vcc actual (tegangan supply Arduino) dalam mV
// Menggunakan internal 1.1V bandgap reference sebagai baseline
// Baca Vcc actual (tegangan supply Arduino) dalam mV
// Menggunakan internal 1.1V bandgap reference sebagai baseline
...
// Baca pH - Simple averaging dengan INTERNAL VREF
// Baca pH - Simple averaging dengan Vcc compensation
// Baca pH - Simple averaging dengan Vcc compensation
// Baca pH - Simple averaging dengan Vcc compensation
void readPH() {
  ...
  if (now - lastPhRead >= PH_READ_INTERVAL) {
    ...
    // Konversi ke voltage - PAKAI g_vcc (set manual di .ino)
    float v = (avg * g_vcc) / 1023.0;
    ...
    if (DEBUG_PH) {
      ...
      Serial.print(v, 3);
      Serial.print(F(" V | Temp="));
      Serial.print((isnan(g_waterTempC) ? 25.0 : g_waterTempC), 1);
      Serial.print(F(" C | pH="));
      Serial.println(pH, 2);
    }
  }
